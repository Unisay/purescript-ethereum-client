...
import Data.Typelevel.Num (class Gt, class LtEq, class Mod, class Pos, type (:*), D0, D1, D2, D24, D32, D5, D6, D8, D80, D19, toInt)

type B160 = D1 :* D6 :* D0
type B256 = D2 :* D5 :* D6

newtype Address = Address (UnsignedInt B160)
instance abiTypeAddress :: AbiType Address where isStatic _ = true
...
class (Pos m, Mod m D8 D0, Gt m D0, LtEq m B256) <= Bitness m

-- | uint<M>: unsigned integer type of M bits, 0 < M <= 256, M % 8 == 0
newtype UnsignedInt m = UnsignedInt Int
instance abiTypeUnsignedInt :: Bitness m => AbiType (UnsignedInt m) where isStatic _ = true

-- | Unsigned n-bit integer: [0, 2^n)
mkUnsignedInt :: ∀ m. Bitness m => m -> Int -> Maybe (UnsignedInt m)
mkUnsignedInt m i
...
instance abiTypeSignedInt :: Bitness m => AbiType (SignedInt m) where isStatic _ = true

-- | Signed n-bit integer: [−2^(n−1), 2^(n−1))
mkSignedInt :: ∀ m. Bitness m => m -> Int -> Maybe (SignedInt m)
...
instance abiTypeSignedFixed :: (Bitness m, From1to80 n) =>
...
instance abiTypeUnsignedFixed :: (Bitness m, From1to80 n) =>
